...
class GetInstagramProfile():
    ...
    def download_users_posts_with_periods(self, username):
        ...
        SINCE = datetime(2022, 11, 25)
        UNTIL = datetime(2022, 11, 26)
    ...
    def get_post_info_csv(self, username):
        with open(username + '.csv', 'w', newline='', encoding='utf-8') as file:
            ...
            for post in takewhile(lambda p: p.date > SINCE, dropwhile(lambda p: p.date > UNTIL, posts)):
                ...
                for comment in post.get_comments():
                    writer.writerow(
                        ["comment", comment.id, comment.owner.username, comment.text.replace(',', ''), comment.created_at_utc])
                    print("comment username: " + comment.owner.username)
                    print("comment text: " + comment.text)
                    print("comment date : " + str(comment.created_at_utc))
                    for answer in comment.answers:
                        writer.writerow(
                            ["comment-answer", answer.id, answer.owner.username, answer.text.replace(',', ''), answer.created_at_utc])
                        print("comment username: " + answer.owner.username)
                        print("comment text: " + answer.text)
                        print("comment date : " + str(answer.created_at_utc))
                    print("\n\n")
                print("\n\n")
...
if __name__ == "__main__":
    ...
    # cls.download_users_posts_with_periods("ootimista")
    ...
    cls.get_post_info_csv("ootimista")
